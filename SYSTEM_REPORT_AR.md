...
| الواجهة | React 19 + Vite 6 + TypeScript + Tailwind (يدوياً عبر CSS متغيّرات) + Lucide + مكوّنات داخلية + Recharts (لبعض الجداول) + MUI Selective components |
...
| `modules/clinics/routes.ts` | `/api/clinics` | تسجيل العيادات، الموافقة/التعليق/الرفض، تعيين التراخيص، Force Logout، وجديدًا الحذف النهائي (Hard Delete) مع تنظيف المستخدمين/المحادثات/الجلسات المرتبطة ضمن معاملة. |
...
| الملف | الغرض |
| --- | --- |
| `hooks/useTranslation.tsx` | نظام ترجمة بسيط يستعمل `localStorage`، EventTarget داخلي لتزامن اللغة، ويحدث اتجاه الصفحة (تم توسيعه ليشمل نصوص العيادات بالكامل). |
...
| الملف | الوظيفة |
| --- | --- |
| `Dashboard.tsx` | لوحة عامة بالبطاقات النصية (تراخيص نشطة/منتهية، إيرادات، صحة الخادم)، نشاط Audit آخر 5 أحداث، وتحديث تلقائي. |
...
| `Clinics.tsx` | إدارة العيادات: عرض القائمة ببحث/فلترة الحالة، شارات واضحة للحالة، تعيين/تحديث التراخيص، الموافقة/التعليق/الرفض، Force Logout، وحذف نهائي. يدعم تحديث تلقائي كل 15 ثانية وترجمة كاملة عربي/إنجليزي. |
| `ClinicDashboard.tsx` | لوحة حية للعيادات: إحصاءات فورية للحالات (Pending/Approved/Suspended/Rejected)، قائمة طلبات حديثة، أشرطة تقدم للخطط، ويدعم التحديث التلقائي واستعراض الإجراءات السريعة للمشرف. |
...
5. **العيادات**: تسجيل الطلبات يتم عبر `/api/clinics/register` بحالة PENDING، ويجري إقرارها/تعليقها/رفضها من لوحة `Clinics`. التغييرات والـ Force Logout تطبق فورًا، واللوحة تحدّث البيانات تلقائيًا لمراقبة الطلبات الجديدة.
...
4. **التحليلات**: بعض الحقول في `analytics/routes.ts` ما زالت تعتمد قيمًا عشوائية للتجارب. للإنتاج يفضّل تغذيتها بقراءات حقيقية أو إخفاؤها.
5. **الحذف النهائي للعيادات**: تم تفعيل Hard Delete في `/api/clinics/:id`. يُنصح بأخذ نسخ احتياطية دورية قبل الحذف لتفادي فقد البيانات غير القابل للاسترجاع.
6. **واجهة العميل المزدوجة**: توجد واجهتان (`/api` و`/client`). تأكد من توحيد سلوكهما أو توثيق الفرق لتفادي التداخل عند تحديثات مستقبلية.
